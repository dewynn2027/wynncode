...
class Appa extends CI_Controller {
	...
	public function __construct()
	{
		...
		$config['functions']['getTransactionDetails'] 	= array('function' => 'Appa.getTransactionDetails');
		$config['functions']['refundapi'] 			= array('function' => 'Appa.refundApi');
		//~ $config['functions']['refundApi'] 			= array('function' => 'Appa.refundApi');
		
		$config['functions']['whipLogin'] 			= array('function' => 'Appa.adminLogin');
		$config['functions']['whipTransHistory'] 		= array('function' => 'Appa.transHistory');
		$config['functions']['whipUserAddEdit'] 		= array('function' => 'Appa.userAddEdit');
		$config['functions']['whipRefund'] 			= array('function' => 'Appa.refund');
		$config['functions']['whipRefundHistory'] 		= array('function' => 'Appa.refundHistory');
	...
	function debug($request="")
	{
		...
		$reqparam = $request;
		$this->psi_model->insert_reqrsp_param("Appa debug",$reqparam,$rsp);
	...
	function adminLogin($request="")
	{
		...
		$reqparam = $request;
		$this->psi_model->insert_reqrsp_param("Appa adminLogin",$reqparam,$rsp);
		return $this->xmlrpc->send_response($rsp);
	...
	function refund($request="")
	{
		...
		$reqparam = $request;
		$this->psi_model->insert_reqrsp_param("Appa refund",$reqparam,$rsp);
		return $this->xmlrpc->send_response($rsp);
	...
	function userAddEdit($request="")
	{
		...
		$reqparam = $request;
		$this->psi_model->insert_reqrsp_param("Appa userAddEdit",$reqparam,$rsp);
		return $this->xmlrpc->send_response($rsp);
	}
	...
	function getTransactionDetails($request="")
	{
		...
		$reqparam = $request;
		$this->psi_model->insert_reqrsp_param("Appa getTransactionDetails",$reqparam,$rsp);
		return $this->xmlrpc->send_response($rsp);
	}
	...
	function paymentApi($request="")
	{
		...
		if($check=="allow")
		{
			...
			if((string)$xml->ReferenceID == "" || (string)$xml->Paymentmethod == "" || (string)$xml->Type == "" || (string)$xml->billNo == "" || (string)$xml->dateTime == "" || (string)$xml->currency == "" || (string)$xml->language == "" || (string)$xml->cardHolderIp == "" || (string)$xml->cardNum == "" || (int)$xml->cvv2 <= 0 || (string)$xml->month == "" || (int)$xml->year <= 0 || (string)$xml->firstName == "" || (string)$xml->lastName == "" || (string)$xml->email=="" || (string)$xml->phone == "" || (string)$xml->zipCode == "" || (string)$xml->address == "" || (string)$xml->city == "" || (string)$xml->state == "" || (string)$xml->country == "")
			{
				...
				$reqparam = $request;
				$this->psi_model->insert_reqrsp_param("Appa paymentApi",$reqparam,$rsp);
				return $this->xmlrpc->send_response($rsp);
				...
				if($checkifexist==0)
				{
					...
					if($dbxml['rc']==1)
					{
						...
						$serverUrl = "https://Appa.biz/eng/ccgate/billing/acquirer/securepay.php";
						...
						{	
							$messageCode = ($responseData[0] == "paymentsuccess") ? 2 : 3;
							$this->psi_model->updateStatus((string)$xml->ReferenceID,(string)$xml->billNo,(string)$xml->cardNum,(string)$responseData[2],$messageCode);
							...
							$condition = (isset($responseData[2])) ? "<paymentOrderNo>".$responseData[2]."</paymentOrderNo>" : "<remarks>".$responseData[0]."</remarks>";
	...
	//~ function refundApi()
	//~ {
		//~ $serverUrl = "http://www.Appa.biz/manage/console/merchants/tui_receive/tui_receive_biz.php";
		//~ $merchantId = (string)$this->merchantId;
	...
	function refundApi($request="")
	{
		...
		$reqparam = $request;
		$this->psi_model->insert_reqrsp_param("Appa refundApi",$reqparam,$rsp);
		return $this->xmlrpc->send_response($rsp);
