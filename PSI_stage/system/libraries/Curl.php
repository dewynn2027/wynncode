...
class Curl {
	...
	CURL SOAP
	==================================================================================*/
	function curlsoap($url,$post_param)
	{
		$soap_do = curl_init();
		...
		curl_setopt($soap_do, CURLOPT_POSTFIELDS,    $post_param); 
		curl_setopt($soap_do, CURLOPT_HTTPHEADER,     array('Content-Type: text/xml; charset=utf-8', 'Content-Length: '.strlen($post_param) ));
		$response =curl_exec($soap_do);
		if($response === false)
		{                
		    $err = '<soap:Envelope><soap:Body>Curl error: ' . curl_error($soap_do).'</soap:Body></soap:Envelope>';
		    curl_close($soap_do);
		    $rsp = $err;
		}
		else
