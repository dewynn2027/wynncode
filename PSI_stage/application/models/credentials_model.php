...
class Credentials_model extends CI_Model {
	...
	function groupAccessAddEdit($groupId=0,$moduleNode,$nodeId,$nodeCount=0)
	{
		...
		$query = $this->db->query("CALL sp_groupAccessAddEdit(".$groupId.",'".$nodeId."',".$nodeCount.")");
		$row = $query->row();
		$result = "<RSP rc='".$row->message_code."' message=\"".$row->message_response."\"></RSP>";
		$this->db->close();
		return $result;
	...
	function apiUser($agt, $appUserId, $keywords, $pageNum, $perPage)
	{
		...
		$query = $this->db->query("SELECT * FROM vw_appuser $where $and  GROUP BY appuserid ORDER BY `appuserid` DESC LIMIT $pageNum, $perPage ");
		$pquery = $this->db->query("SELECT appuserid FROM vw_appuser $where $and  GROUP BY appuserid");
		if($this->db->_error_number()==0)
		{
			$result  = "<response rc='0' message='success'>";
			foreach($query->result_array() as $k => $v)
				...
				if(is_array($v))
				{
					...
					foreach($v as $key => $val)
					{
						...
						if($key!="")
						{
							...
							if($key!="dtime_created" && $key!="inputed")
							{
