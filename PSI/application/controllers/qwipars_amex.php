...
class Qwipars_amex extends CI_Controller {
	...
	public function __construct()
	{
		...
		$config['functions']['paymentapi'] 			= array('function' => 'Qwipars_amex.paymentApi');
	...
	function paymentApi($request="")
	{
		...
		}elseif((string)$xml->referenceId == "" || (string)$xml->billNo == "" || (string)$xml->dateTime == "" || (string)$xml->currency == "" || (string)$xml->language == "" || (string)$xml->cardHolderIp == "" || (string)$xml->cardNum == "" || (int)$xml->cvv2 <= 0 || (string)$xml->month == "" || (int)$xml->year <= 0 || $xml->birthDate == "" || (string)$xml->firstName == "" || (string)$xml->lastName == "" || (string)$xml->email=="" || (string)$xml->phone == "" || (string)$xml->zipCode == "" || (string)$xml->address == "" || (string)$xml->city == "" || (string)$xml->state == "" || (string)$xml->country == "" || $xml->loginName == "")
		{
			...
			if($checkifexist==0)
			{
				...
				if($dbxml['rc']==1)
				{
					...
					$this->logme("RequestParameter:  referenceId: ".$xml->referenceId,"QwiparsAmexpaymentApi");
					$this->logme((string)$url.$fparams,"QwiparsAmexpaymentApi");
					$xmlrsp = $this->whip_model->sendrequest($url , $fparams);
					$this->logme("ResponseParameter:  referenceId: ".$xml->referenceId,"QwiparsAmexpaymentApi");
					$this->logme((string)$xmlrsp,"QwiparsAmexpaymentApi");
		...
		$reqparam = $request;
		$this->nginv2_model->reqrspLogs((string)$_SERVER["REMOTE_ADDR"],"QwiparsAmex paymentApi",$reqparam,$rsp);
