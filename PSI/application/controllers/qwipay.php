...
class Qwipay extends CI_Controller {
	...
	public function __construct()
	{
		...
		$config['functions']['debug'] 				= array('function' => 'Qwipay.debug');
		$config['functions']['paymentapi'] 			= array('function' => 'Qwipay.paymentApi');
		$config['functions']['refundapi'] 			= array('function' => 'Qwipay.paymentRefundArs');
	...
	function paymentRefundArs($request="")
	{
		...
		if($check=="allow")
		{
			...
			if($resultdb['rc']==0)
			{
				$card = $this->psidb_model->getCardNumber((string)$xml->paymentOrderNo);
				...
				if($getCardDb['rc']!=999)
				{
					...
					$this->logme("RequestParameter:  referenceId: ".$xml->referenceId,"QwipayRefundApi");
					$this->logme((string)$url.$fparams,"QwipayRefundApi");
					$xmlrsp = $this->whip_model->sendrequest($url , $fparams);
					$this->logme("ResponseParameter:  referenceId: ".$xml->referenceId,"QwipayRefundApi");
					$this->logme((string)$xmlrsp,"QwipayRefundApi");
		...
		$reqparam = $request;
		$this->psidb_model->reqrspLogs((string)$_SERVER["REMOTE_ADDR"],"Qwipay paymentRefundArs",$reqparam,$rsp);
	...
	function paymentApi($request="")
	{
		...
		if((string)$xml->referenceId == "" || (string)$xml->billNo == "" || (string)$xml->dateTime == "" || (string)$xml->currency == "" || (string)$xml->language == "" || (string)$xml->cardHolderIp == "" || (string)$xml->cardNum == "" || (int)$xml->cvv2 <= 0 || (string)$xml->month == "" || (int)$xml->year <= 0 || $xml->birthDate=="" || (string)$xml->firstName == "" || (string)$xml->lastName == "" || (string)$xml->email=="" || (string)$xml->phone == "" || (string)$xml->zipCode == "" || (string)$xml->address == "" || (string)$xml->city == "" || (string)$xml->state == "" || (string)$xml->country == "")
		{
			...
			$msg .= (empty($xml->birthDate)) ? "birthDate(YYYYMMDD), " : "";
		...
		}else if(strlen($xml->month) != 2){
		
			$rsp = "<response rc='999' message='month format should be MM.'></response>";
			
		}else if(strlen($xml->year) != 4){
		
			$rsp = "<response rc='999' message='year format should be YYYY.'></response>";
			
		}else if(strlen($xml->birthDate) != 8){
		
			$rsp = "<response rc='999' message='birthDate format should be YYYYMMDD.'></response>";
			
		}else if(!filter_var($xml->email, FILTER_VALIDATE_EMAIL)){
		
			$rsp = "<response rc='999' message='Email format incorrect.'></response>";
			...
			if($checkifexist==0)
			{
				...
				if($dbxml['rc']==1)
				{
					...
					$card = (string)$xml->cardNum;
					$md5Ikey = ($card[0]==3) ? $this->config->item('qwiparsM5key'.$card[0]) : $this->config->item('qwipaymd5Key');
					$merNo = ($card[0]==3) ? $this->config->item('qwiparsMID'.$card[0]) : $this->config->item('qwipayMID');
					$url = ($card[0]==3) ? str_replace(" ", "%20", $this->config->item('qwiparsPaymentServerUrl'.$card[0])) : str_replace(" ", "%20", $this->config->item('qwipayPaymentServerUrl'));
					...
					$fparams = str_replace(" ", "%20", $params);
					$this->logme("RequestParameter:  referenceId: ".$xml->referenceId,"QwipayPaymentApi");
					$this->logme((string)$url.$fparams,"QwipayPaymentApi");
					$xmlrsp = $this->whip_model->sendrequest($url , $fparams);
					$this->logme("ResponseParameter:  referenceId: ".$xml->referenceId,"QwipayPaymentApi");
					$this->logme((string)$xmlrsp,"QwipayPaymentApi");
					...
					if($result->resultCode==88)
					{
						...
						$rsp = "<response rc='0' message='".$this->config->item('qwipayerror'.$result->resultCode)."'>";
						$rsp .= "<referenceId>".$xml->referenceId."</referenceId>";
						...
						$rsp = "<response rc='999' message='".$this->config->item('qwipayerror'.$result->resultCode)."'>";
						$rsp .= "<referenceId>".$xml->referenceId."</referenceId>";
						$rsp .= "<billNo>".$xml->billNo."</billNo>";
						$rsp .= "<currency>".$result->currency."</currency>";
						$rsp .= "<amount>".$result->amount."</amount>";
		...
		$reqparam = $request;
		$this->psidb_model->reqrspLogs((string)$_SERVER["REMOTE_ADDR"],"Qwipay qwipaypaymentapi",$reqparam,$rsp);
 		return $this->xmlrpc->send_response($rsp);
	}
